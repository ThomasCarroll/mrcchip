...
with open(csvFile, 'rb') as csvfile:
 ...
 for row in spamreader:
  ...
  if row[3] in SampleList:
   ...
   submitCmd5 = "echo "+os.path.join(baseForPipeline,"runMotif.py")+" "+row[0]+" "+row[1]+" "+row[2]+" "+row[3]+" "+baseDir+" | qsub -l select=1:ncpus=1:mem=12GB -l walltime=70:00:00 -W depend=afterok:"+stdoutMacsPeakCallDict[jobName].strip()+" -e "+errorPath+" -o "+outputPath
   p5 = subprocess.Popen(["/bin/bash",'-i',"-c",submitCmd5],stdout=subprocess.PIPE,stderr=subprocess.PIPE)
   stdout5,stderr5 = p5.communicate() 
   #stdoutMacsPeakCallDict[jobName] = stdout5
   print submitCmd5
   f.write(stdout5.strip()+"\t"+submitCmd5+"\n")          
